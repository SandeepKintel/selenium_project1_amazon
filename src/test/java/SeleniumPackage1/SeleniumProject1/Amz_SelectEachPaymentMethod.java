...
public class Amz_SelectEachPaymentMethod extends Amz_LaunchAndQuit
{
	@Test(dataProvider="inputvalue", retryAnalyzer=SeleniumPackage1.SeleniumProject1.IRetryAnalyzerPro.class)
	public void SelectPaymentMode(String value1) throws EncryptedDocumentException, IOException, InterruptedException
	{
		...
		cp.ClickCreditorDebitCardPaymentMode();
		wait.until(ExpectedConditions.elementToBeSelected(cp.CreditOrDebitCard));
		Assert.assertTrue(cp.CreditOrDebitCard.isSelected());
		cp.ClickNetBankingPaymentMode();
		wait.until(ExpectedConditions.elementToBeSelected(cp.NetBanking));
		Assert.assertTrue(cp.NetBanking.isSelected());
		cp.ClickUPIPaymentMode();
		wait.until(ExpectedConditions.elementToBeSelected(cp.UPIMode));
		Assert.assertTrue(cp.UPIMode.isSelected());
		cp.ClickEMIPaymentMode();
		wait.until(ExpectedConditions.elementToBeSelected(cp.EMIMode));
		Assert.assertTrue(cp.EMIMode.isSelected());
		cp.ClickCODPaymentMode();
		wait.until(ExpectedConditions.elementToBeSelected(cp.CODMode));
		Assert.assertTrue(cp.CODMode.isSelected());
